...
export function GenericDepartmentContent({
  program, faculty, heroImage, labs, labImages, alumniImages, videos, syllabus, notices, contactHod
}: GenericDepartmentProps) {
      ...
      <div className="relative h-[60vh] min-h-[400px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="w-full max-w-7xl mx-auto px-6 pb-12">
            <Link href="/academics" className="inline-flex items-center gap-2 text-sm text-white/70 hover:text-white transition-colors mb-6">
        ...
        {activeTab === "overview" && (
          <div className="space-y-12">
            <div>
              ...
              <p className="text-gray-600 leading-relaxed text-lg max-w-4xl">{program.description}</p>
            </div>
            <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {program.relatedTabs.map((tab, i) => (
                    <a key={i} href={tab.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-5 py-4 text-gray-600 bg-gray-50 hover:bg-[#00274C] hover:text-white transition-colors border border-gray-100">
                ...
                <ol className="space-y-6 list-decimal list-inside">
                  {program.peos.map((peo, i) => (
                    <li key={i} className="text-gray-600 pl-2">
                      <span className="font-bold text-[#00274C]">PEO{i + 1}:</span> <span className="text-lg leading-relaxed">{peo}</span>
                    </li>
                  ))}
                </ol>
              ) : (
        ...
        {activeTab === "labs" && (
          <div className="space-y-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                  {labImages.map((img, i) => (
                    <div key={i} className="aspect-[4/3] overflow-hidden bg-gray-100">
                      <img src={img} alt={`Lab ${i + 1}`} className="w-full h-full object-cover" loading="lazy" />
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {labs.map((lab) => (
                  <div key={lab} className="px-5 py-4 bg-gray-50 border border-gray-100 text-gray-700">{lab}</div>
                ))}
              </div>
            </div>
          </div>
        ...
        {activeTab === "alumni" && (
          <div>
            ...
            {alumniImages.length > 0 ? (
              ...
              <p className="text-gray-500">Alumni photos will be updated soon.</p>
            )}
          </div>
        )}
                ...
                {syllabus.map((item) => (
                  <div key={item.file} className="bg-gray-50 border border-gray-100">
                    <div className="flex items-center justify-between p-6">
                      ...
                      <a href={item.file} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-[#00274C] bg-white border border-[#00274C] hover:bg-[#00274C] hover:text-white transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {videos.map((video) => (
                  <div key={video.src} className="bg-gray-50 border border-gray-100 p-4">
                    <video src={video.src} className="w-full aspect-video object-cover" controls preload="metadata" />
                    <p className="text-sm font-bold text-[#00274C] mt-3">{video.title}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-gray-500">Innovative teaching videos will be updated soon.</p>
            )}
          </div>
        )}
